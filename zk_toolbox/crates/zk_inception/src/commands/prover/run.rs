...
fn run_prover(shell: &Shell, chain: &ChainConfig) -> anyhow::Result<()> {
    ...
    let mut cmd = Cmd::new(
        cmd!(shell, "cargo run --release --bin zksync_prover_fri -- --config-path={config_path} --secrets-path={secrets_path}"),
    );
    cmd = cmd.with_force_run();
...
fn run_compressor(
    ...
    let mut cmd = Cmd::new(cmd!(shell, "cargo run --release --bin zksync_proof_fri_compressor -- --config-path={config_path} --secrets-path={secrets_path}"));
    cmd = cmd.with_force_run();
